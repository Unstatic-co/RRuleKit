...
public struct RecurrenceDayOfWeek: Hashable {
    ...
    ///
    /// Values range from `-53` to `53`. A negative value indicates a value from the end of the range. `0` indicates the
    /// week number is irrelevant.
...
public struct RecurrenceRule: Hashable {
    ...
    /// Specifies how often the recurrence rule repeats over the unit of time indicated by its frequency. For example, a
    /// recurrence rule with a frequency type of `.weekly` and an interval of `2` repeats every two weeks.
    public var interval: Int
    /// Indicates which day of the week the recurrence rule treats as the first day of the week. No value indicates that
    /// this property is not set for the recurrence rule.
    ...
    /// The days of the month associated with the recurrence rule, as an array of `Int`. Values can be from 1 to 31 and
    /// from -1 to -31. This property value is invalid with a frequency type of `.weekly`.
    public var daysOfTheMonth: [Int]
    /// The days of the year associated with the recurrence rule, as an array of `Int`. Values can be from 1 to 366 and
    /// from -1 to -366. This property value is valid only for recurrence rules initialized with a frequency type of
    /// `.yearly`.
    public let daysOfTheYear: [Int]?
    /// The weeks of the year associated with the recurrence rule, as an array of `Int` objects. Values can be from 1 to
    /// 53 and from -1 to -53. This property value is valid only for recurrence rules initialized with specific weeks of
    /// the year and a frequency type of `.yearly`.
    public let weeksOfTheYear: [Int]?
    /// The months of the year associated with the recurrence rule, as an array of `Int` objects. Values can be from 1
    /// to 12. This property value is valid only for recurrence rules initialized with specific months of the year and a
    /// frequency type of `.yearly`.
    public var monthsOfTheYear: [Int]
    /// An array of ordinal numbers that filters which recurrences to include in the recurrence rule’s frequency. For
    /// example, a yearly recurrence rule that has a daysOfTheWeek value that specifies Monday through Friday, and a
    /// setPositions array containing 2 and -1, occurs only on the second weekday and last weekday of every year.
    ...
    ///   - interval: The interval between instances of this recurrence. For example, a weekly recurrence rule with an
    /// interval of `2` occurs every other week. Must be greater than `0`.
    ///   - days: The days of the week that the event occurs, as an array of `RecurrenceDayOfWeek` objects.
    ///   - monthDays: The days of the month that the event occurs, as an array of `Int`. Values can be from 1 to 31 and
    /// from -1 to -31. This parameter is not valid for recurrence rules of type `.weekly`.
    ///   - months: The months of the year that the event occurs, as an array of `Int`. Values can be from 1 to 12.
    ///   - weeksOfTheYear: The weeks of the year that the event occurs, as an array of `Int`. Values can be from 1 to
    /// 53 and from -1 to -53. This parameter is only valid for recurrence rules of type `.yearly`.
    ///   - daysOfTheYear: The days of the year that the event occurs, as an array of `Int`. Values can be from 1 to 366
    /// and from -1 to -366. This parameter is only valid for recurrence rules of type `.yearly`.
    ///   - setPositions: An array of ordinal numbers that filters which recurrences to include in the recurrence rule’s
    /// frequency. See `setPositions` for more information.
