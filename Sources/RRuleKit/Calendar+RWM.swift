...
public extension Calendar {
    ...
    func range(of weekday: Int, in year: Int, month: Int = 0) -> ClosedRange<Int>? {
        if month > 0 {
            ...
            if let date = date(from: comps) {
                let count = component(.weekdayOrdinal, from: date)
            ...
            if let dateStart = date(from: startComps), let finishDate = date(from: finishComps) {
                // Get the number of days between the two dates
                let days = dateComponents([.day], from: dateStart, to: finishDate).day!
            ...
            if let weekday = components.weekday, let ordinal = components.weekdayOrdinal {
                if ordinal < 0 {
                    if let month = components.month {
                        if let rng = range(of: weekday, in: year, month: month) {
                            newComponents.weekdayOrdinal = rng.count + ordinal + 1
                        }
                    } else {
                        if let rng = range(of: weekday, in: year) {
                            newComponents.weekdayOrdinal = rng.count + ordinal + 1
                        }
                    }
                } else {
                ...
                if components.weekday == nil {
                    if let month = components.month {
                        if day < 0 {
                            if let startOfMonth = date(from: DateComponents(year: year, month: month, day: 1)),
                               let daysInMonth = range(of: .day, in: .month, for: startOfMonth)?.count
                            {
                        ...
                        if day < 0 {
                            if let startOfYear = date(from: DateComponents(year: year, month: 1, day: 1)),
                               let daysInYear = range(of: .day, in: .year, for: startOfYear)?.count
                            {
    ...
    func date(fromRelative components: DateComponents) -> Date? {
        date(from: relativeComponents(from: components))
    }

    func date(_ date: Date, matchesRelativeComponents components: DateComponents) -> Bool {
        self.date(date, matchesComponents: relativeComponents(from: components))
    }
}
